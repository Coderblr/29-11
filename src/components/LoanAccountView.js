...
export default function LoanAccountView() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            // Add to history
            addToHistory('Loan Account', {
                count: parseInt(no_acc, 10),
                region: region,
                cif: cif,
                details: `${parseInt(no_acc, 10)} Loan Account(s) created for CIF ${cif}`
            });
        } catch (err) {
    ...
    const downloadCsv = () => {
        ...
        link.setAttribute("download", "loan_account_data.csv");
